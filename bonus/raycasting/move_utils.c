...
void move_player(t_jeux *jeux)
{
    ...
    if (jeux->keys.up) 
    {
        next_x = x_player + next_movecos;
        next_y = y_player + next_movesin;
    }
    if (jeux->keys.down) 
    {
        next_x = x_player - next_movecos;
    ...
    if (jeux->keys.right) 
    {
        next_x = x_player - next_movesin;
        next_y = y_player + next_movecos;
    }
    if (jeux->keys.left) 
    {
        next_x = x_player + next_movesin;
    ...
    if (no_walls(next_x, y_player, jeux->cub.cub_map))
        jeux->cub.player.player_x = next_x;
    if (no_walls(x_player, next_y, jeux->cub.cub_map))
